/**
 * Comprehensive eval runner for the HAR reverse-engineering pipeline.
 *
 * Tests the full pipeline: parse -> filter -> summarize -> LLM match
 * across diverse real-world scenarios with varying difficulty levels.
 *
 * Run:
 *   cd backend && npx jest eval --testTimeout=120000 --verbose
 *
...
afterAll(() => {
  ...
  console.log('  COMPREHENSIVE EVAL SUMMARY');
  console.log('='.repeat(90));
...
describe('HAR Analysis Eval', () => {
  test.each(evalCases)(
    ...
    async ({ fixture, description, expectedUrlPattern, expectedBodyPattern, category, difficulty }: EvalCase) => {
      ...
      const llmResult = await openai.identifyApiRequest(
        llmSummary,
        description,
        filtered.length,
      );
